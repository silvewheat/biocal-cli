...
@click.option('--popc', help='群体C(候选的渗入来源群体)的ID列表，一行一个')
@click.option('--popd', help='群体D(需要排除的渗入来源群体)的ID列表，一行一个')
...
def main(vcffile, popa, popb, popc, popd, binwidth, stepsize, outprefix):
    """
    U_A,B,C(w,x,y)
    ...
    samples_popD = [x.strip() for x in open(popd)]
    samples_queried = samples_popA + samples_popB + samples_popC + samples_popD
    ...
    selection_popD = select_samples(samples_all, samples_popD)
    af_popD = cal_alt1_freq(gt_array[:, selection_popD, :]) # 这个返回的是第一个ALT的frequency
    # 只保留群体C中频率百分百且在D中百分之零的位点, ALT按popC转换过了，所以直接算ALT的频率
    selection_freq_popC = af_popC == 1 # 在popC中频率100%
    selection_freq_popD = af_popD == 0 # 在popD中频率为0%
    selection_freq_popCD = selection_freq_popC & selection_freq_popD
    gt_array = gt_array[selection_freq_popCD, :, :]
    pos_array = pos_array[selection_freq_popCD]
    chrom_array = chrom_array[selection_freq_popCD]
    af_popC = af_popC[selection_freq_popCD]
    af_popD = af_popD[selection_freq_popCD]
    ...
    # 先把频率文件保存一下
    df = pd.DataFrame({'chrom': chrom_array, 'pos': pos_array, 'popA': af_popA, 'popB': af_popB, 'popC': af_popC, 'popD': af_popD})
    ...
    selection_popD_1percent = df['popD'].values == 0
    df = df.iloc[selection_popD_1percent, :]
    ...
    for offset in range(0, binwidth, stepsize):
        df[f'bin_index'] = ((df['pos'].values - 1) - offset) // binwidth
