...
def main(vcffile: str = typer.Argument(..., help="总vcf文件"),
         outprefix: str = typer.Argument(..., help='输出文件名前缀'),
    ...
    for nregion, region in enumerate(regions, 1):
        print(region)

        H_query, samples, chrs, sites = vcf2npy(vcffile, querysamples, region=region, return_more=True)
        if nregion == 1:
            hapIDs = [[f'{i}_1', f'{i}_2'] for i in samples]
            hapIDs = [i for j in hapIDs for i in j]
            cols = ['chrom', 'pos', 'region'] + hapIDs
            header = '\t'.join(cols) + '\n'
            with gzip.open(f'{outprefix}.anc.tsv.gz', 'wb') as f:
                f.write(header.encode())
            with gzip.open(f'{outprefix}.bag.tsv.gz', 'wb') as f:
                f.write(header.encode())
        ...
        df['chrom'] = chrs
        df['pos'] = sites
        df['region'] = region # 这个很关键，因为是分区域算的，区域之间可能不连续，后续位点合并为区间只在一个区域内进行
        df[cols].to_csv(f'{outprefix}.anc.tsv.gz', sep='\t', index=False, header=None, mode='a', compression='gzip')
        # bagging最多票数
        df = pd.DataFrame(res_loter[1].T, columns=hapIDs)
        df['chrom'] = chrs
        df['pos'] = sites
        df['region'] = region
        df[cols].to_csv(f'{outprefix}.bag.tsv.gz', sep='\t', index=False, header=None, mode='a', compression='gzip')
